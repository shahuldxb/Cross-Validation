...
if st.button("Analyze"):
    ...
    # --- Improved semantic classification prompt ---
    prompt = f"""
You are an AI document classifier specialized in international trade finance.

You will be given a block of text that may contain multiple trade documents joined together
(such as Letter of Credit, Commercial Invoice, Packing List, Bill of Lading, Insurance Certificate, etc.).

Your task:
1. Split the text into individual document snippets.
2. For each snippet, detect its **document type** intelligently — not just by name matching.
3. Identify it even if it uses synonyms or variants, e.g.:
   - "Ocean Bill of Lading", "B/L", "Shipped on Board" → Bill of Lading
   - "Insurance Policy", "Cargo Insurance", "Institute Cargo Clauses" → Insurance Certificate
   - "Invoice", "Commercial Invoice" → Commercial Invoice
   - "Packing List", "Weight List" → Packing List
   - "Certificate of Origin", "COO" → Certificate of Origin
   - "Quality Certificate", "Inspection Certificate" → Quality Certificate
   - "Letter of Credit", "Documentary Credit", "LC" → Letter of Credit

Here is a reference list of standard trade finance document types:
{document_list}
...
[
  {{
    "heading": "detected heading or first line summary",
    "content": "full snippet text",
    "identified_name": "AI-determined document name (from above list or best match)"
  }},
  ...
]

If no close match exists, use "Unknown".
...
"""
    ...
    try:
        ...
        # --- Normalize AI-identified names for consistency ---
        def normalize_doc_name(name):
            ...
            if "packing" in n or "weight list" in n:
                return "Packing List"
        ...
        for item in data:
            item["identified_name"] = normalize_doc_name(item.get("identified_name"))
