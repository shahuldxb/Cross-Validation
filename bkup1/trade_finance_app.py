...
def split_secondary_documents(content):
    ...
    # Use the exact prompt from the working file with better document matching
...
For each snippet, identify the document name from the list that best matches the content. Use these exact names:
- For insurance documents: "Insurance Certificate"
- For bill of lading documents: "Bill of Lading"
- For invoice documents: "Commercial Invoice"
- For packing documents: "Packing List"
- For certificate documents: "Certificate of Origin"
- For letter of credit documents: use the appropriate LC type from the list
...
"""
    ...
    try:
        ...
        logger.debug(f"AI response: {result[:200]}...")
...
def save_secondary_documents(documents):
    ...
    for i, doc in enumerate(documents, 1):
        # Get identified name for better filename
        identified_name = doc.get('identified_name', 'Unknown')
        heading = doc['heading']
        
        # Use the exact heading as filename (as requested)
        filename = f"{heading}.txt"
        filepath = Path("input") / filename
...
def determine_document_type_for_heading(identified_name, filename):
    ...
    # Match the logic from cross_new_csv.py determine_document_type function
    if any(x in identified_lower for x in ['bill of lading', 'bol', 'bill', 'lading']):
        return "Bill of Lading (BOL)"
    elif any(x in identified_lower for x in ['insurance', 'ins', 'certificate']):
    ...
    elif any(x in identified_lower for x in ['certificate of origin', 'certificate', 'cert', 'origin']):
...
def main():
        ...
        if secondary_content.strip():
            with st.expander("📋 Preview Document Splitting"):
                with st.spinner("Analyzing documents..."):
                    ...
                    if preview_docs:
                        st.success(f"✅ Found {len(preview_docs)} documents")
                        for i, doc in enumerate(preview_docs, 1):
                            st.write(f"**Document {i}:** {doc['heading']}")
                            st.write(f"**Identified as:** {doc.get('identified_name', 'Unknown')}")
    ...
    if st.button("🔍 Analyze Documents", type="primary", use_container_width=True):
        ...
        # Split and save secondary documents
        ...
        saved_secondary = save_secondary_documents(secondary_docs)
        ...
        with st.spinner("Running discrepancy analysis..."):
            ...
            if not md_file or not csv_file:
                ...
                if csv_files:
                    csv_file = max(csv_files, key=os.path.getctime)
                    logger.info(f"Found analysis CSV file despite error: {csv_file}")
